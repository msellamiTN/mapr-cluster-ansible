...
[ -z "${MAPR_HOME}" ] && exit_fail "MAPR_HOME not set"
[ -n "${IS_UPDATE}" ] && [ "$mapr_version" -gt 510 ] && RECALCOPT=" --noRecalcMem "
if which maprcli &> /dev/null ; then
    ...
    $TIMEOUT ${MAPR_HOME}/server/configure.sh $RECALCOPT -R  &> $logfile
    CONF_RET=$?
    check_timeout "configure.sh" $CONF_RET
    exit_with_cmd_ret $CONF_RET
else
    exit_no_change "configure.sh refresh skipped as maprcli not found"
fi
