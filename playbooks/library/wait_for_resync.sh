...
TIMEOUT_MRCONFIG=${TIMEOUT_MRCONFIG:-2}
get_timeout $TIMEOUT_MRCONFIG
LOG_RUN_CMD "$TIMEOUT ${MAPR_HOME}/server/mrconfig info containers resync local | egrep -v '^-|^Time|^$'"
if [ $CMD_RET -gt 1 ] || [ -n "$CMD_RES" ]; then
    exit_fail "$script_name Resync of local containers is in progress"
fi
