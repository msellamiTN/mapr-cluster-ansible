...
[ -z "${MAPR_GID}" ] && exit_fail "MAPR_USER required"
[ -z "${MAPR_GROUP}" ] && exit_fail "MAPR_GROUP required"
LOG_RUN_CMD grep -i "^${MAPR_GROUP}:" /etc/group > /dev/null 2>&1
if [ $CMD_RET -ne 0 ]; then
    LOG_RUN_CMD groupadd -g ${MAPR_GID} ${MAPR_GROUP}
    exit_with_cmd_ret
else
    exit_no_change "Group ${MAPR_GROUP} already exists"
fi
